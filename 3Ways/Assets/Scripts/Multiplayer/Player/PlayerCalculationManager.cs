...
public class PlayerCalculationManager : MonoBehaviourPunCallbacks
{
    ...
    void Start()
    {
        ...
        isPlayer1 = controller.isMasterClient;
        GetValues(1);

        currentDifficulty = PlayerInfo.PI.gameRoomDifficulty;

        PV.RPC("CreateEquation", RpcTarget.All, currentDifficulty, MS.currentLevel);
    ...
    private void NotifyForException(int level)
    {
        if (level < 0 || level > 10)
        {
            Debug.LogWarning("<color=red>level num to create equation was out of range</color>");
    ...
    private void SET_AND_CALCULATE(int aMin, int aMax, string operation, int bMin = 0, int bMax = 0, int deMin = 0, int deMax = 0)
    {
        ...
        else if (operation == SQUAREROOT)
        {
            ...
            if (!bb)
            {
                Debug.Log("Bad Number for rooting! <color=red>:</color><color=yellow>)</color");
                SET_AND_CALCULATE(aMin, aMax, operation, bMin, bMax);
            }
        }
    ...
    private void GetValuesSection1(bool bPlayer1)
    {
        ...
        TextMesh equationTMesh1 = GameObject.FindGameObjectWithTag("equation").GetComponent<TextMesh>();
        TextMesh equationTMesh2 = GameObject.FindGameObjectWithTag("equation2").GetComponent<TextMesh>();

        equationText = bPlayer1 ? equationTMesh1 : equationTMesh2;

        TextMesh result0TMesh1 = GameObject.FindGameObjectWithTag("result0").GetComponent<TextMesh>();
        TextMesh result0TMesh2 = GameObject.FindGameObjectWithTag("2result0").GetComponent<TextMesh>();

        result0Text = bPlayer1 ? result0TMesh1 : result0TMesh2;

        TextMesh result1TMesh1 = GameObject.FindGameObjectWithTag("result1").GetComponent<TextMesh>();
        TextMesh result1TMesh2 = GameObject.FindGameObjectWithTag("2result1").GetComponent<TextMesh>();

        result1Text = bPlayer1 ? result1TMesh1 : result1TMesh2;

        TextMesh result2TMesh1 = GameObject.FindGameObjectWithTag("result2").GetComponent<TextMesh>();
        TextMesh result2TMesh2 = GameObject.FindGameObjectWithTag("2result2").GetComponent<TextMesh>();

        result2Text = bPlayer1 ? result2TMesh1 : result2TMesh2;
    }

    private void GetValuesSection2(bool bPlayer1)
    {
        TextMesh equationTMesh1 = GameObject.FindGameObjectWithTag("equation1").GetComponent<TextMesh>();
        TextMesh equationTMesh2 = GameObject.FindGameObjectWithTag("2equation1").GetComponent<TextMesh>();

        equationText = bPlayer1 ? equationTMesh1 : equationTMesh2;

        TextMesh result0TMesh1 = GameObject.FindGameObjectWithTag("result0_1").GetComponent<TextMesh>();
        TextMesh result0TMesh2 = GameObject.FindGameObjectWithTag("2result0_1").GetComponent<TextMesh>();

        result0Text = bPlayer1 ? result0TMesh1 : result0TMesh2;

        TextMesh result1TMesh1 = GameObject.FindGameObjectWithTag("result1_1").GetComponent<TextMesh>();
        TextMesh result1TMesh2 = GameObject.FindGameObjectWithTag("2result1_1").GetComponent<TextMesh>();

        result1Text = bPlayer1 ? result1TMesh1 : result1TMesh2;

        TextMesh result2TMesh1 = GameObject.FindGameObjectWithTag("result2_1").GetComponent<TextMesh>();
        TextMesh result2TMesh2 = GameObject.FindGameObjectWithTag("2result2_1").GetComponent<TextMesh>();
